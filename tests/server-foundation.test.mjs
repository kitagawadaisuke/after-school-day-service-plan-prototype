...
test("支援職員は職員・事業所・操作履歴の管理権限を持たない", async () => {
  const config = testConfig({ devActor: { ...testConfig().devActor, role: "support_staff" } });
  const app = await buildApp({ config, pool: null });
...
test("支援職員は日誌・連絡帳の削除処理へ進める", async () => {
  const config = testConfig({ devActor: { ...testConfig().devActor, role: "support_staff" } });
  const app = await buildApp({ config, pool: null });
...
test("支援職員は利用者を一覧から削除する処理へ進める", async () => {
  const config = testConfig({ devActor: { ...testConfig().devActor, role: "support_staff" } });
  const app = await buildApp({ config, pool: null });
  const response = await app.inject({
    method: "DELETE",
