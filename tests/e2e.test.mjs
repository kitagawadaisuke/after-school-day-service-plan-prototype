...
function pdfPageCount(pdfPath) {
  ...
  const pythonResult = spawnSync(command, args, { encoding: "utf8" });
  if (pythonResult.status === 0) return Number(pythonResult.stdout.trim());

  const pdfInfo = spawnSync("pdfinfo", [pdfPath], { encoding: "utf8" });
  assert.equal(
    pdfInfo.status,
    0,
    `PDFページ数を確認できません: ${pythonResult.stderr || pdfInfo.stderr}`,
  );
  const match = pdfInfo.stdout.match(/^Pages:\s*(\d+)$/m);
  assert.ok(match, `pdfinfoからページ数を取得できません: ${pdfInfo.stdout}`);
