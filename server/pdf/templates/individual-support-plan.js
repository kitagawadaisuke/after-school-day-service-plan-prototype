import { buildCocoForm, cell, payloadPlanRows, payloadScheduleCells, planRows, section, signedDate, text, value } from "./coco-form.js";
...
export function renderIndividualSupportPlan(source, snapshotKind) {
  ...
  const supportGoals = storedSupportGoals.length ? storedSupportGoals : payloadPlanRows(payload);
  const bodyHtml = [
    ...
    section("具体的な支援内容", `<table class="coco-table goal-table"><thead><tr><th>支援目標</th><th>具体的な支援内容（活動プログラム）</th><th>達成時期</th><th>5領域</th></tr></thead><tbody>${planRows(supportGoals)}</tbody></table>`),
    section("週間計画", `<table class="coco-table daily-table"><thead><tr>${["月", "火", "水", "木", "金", "土", "日", "学校休業"].map((day) => `<th>${day}</th>`).join("")}</tr></thead><tbody><tr>${payloadScheduleCells(payload, source.schedules)}</tr></tbody></table><div class="coco-grid one">${cell("利用時間帯・その他", value(payload, ["weeklyNotes", "serviceDelivery", "supportConsiderations"]), "tall")}${cell("留意事項", value(payload, ["supportConsiderations", "explanationNotes"]), "tall")}</div>`),
